...
const tokenRef = useCookie<{ jwt: string; decoded: object } | undefined>('token')
const url = useRuntimeConfig().public.apiUrl

const { data: prestataireData, error } = await useFetch(`${url}/prestataire/me`, {
	headers: {
		Authorization: `Bearer ${tokenRef.value?.jwt}`
	}
})

const prestataire = computed(() => prestataireData.value?.data?.prestataire)
const user = computed(() => prestataireData.value?.data?.users)
